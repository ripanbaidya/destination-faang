 ...
 * the longest subarray where the sum of its elements is equal to the given value k. If there is
 * no subarray with sum equal to k, return 0.
 *
 * Example:
 * Input: arr[] = [10, 5, 2, 7, 1, -10], k = 15
 * Output: 6
 * Explanation: Subarrays with sum = 15 are [5, 2, 7, 1], [10, 5] and [10, 5, 2, 7, 1, -10]. The
 * length of the longest subarray with a sum of 15 is 6.
 */
public class LongestSubarrayWithSumK {
    // This code will work for both positive and negative integers in array.
    public int longestSubarray(int[] arr, int k) {
        int n = arr.length;
        int longest = 0, prefSum = 0;
