 ...
 * @date 31-07-2025
 *
 * Given a Graph with V vertices (Numbered from 0 to V-1) and E edges. Check whether the
 * graph is bipartite or not. A bipartite graph can be colored with two colors such that
 * no two adjacent vertices share the same color. This means we can divide the graph’s
 * vertices into two distinct sets where:
 *
 * All edges connect vertices from one set to vertices in the other set. No edges exist
 * between vertices within the same set.
 *
 * Example:
 *
...
public class BipartiteGraph {
    ...
    public static void main(String[] args) {
        var obj = new BipartiteGraph();

        int V = 3;
        int[][] edges = {{0, 1}, {1, 2}}; // bipartite graph

        boolean isBipartiteGraph = obj.isBipartite(V, edges);
